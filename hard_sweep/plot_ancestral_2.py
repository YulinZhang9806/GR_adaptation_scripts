...
for run in range(0,4):
	...
	out3=[]
	for l in range(1,11):
		infile=open('admix_MI0.001hard_recessive'+str(hset[run])+'_model2_run1.out')
...
out3=np.array(out3)
np.savetxt('ancestryGenome_MI0.001hard_recessive_admix.txt',out3,fmt='%s',delimiter='\n',newline='\n')
...
for run in range(0,4):
	...
	out3=[]
	for l in range(1,11):
		infile=open('admix_MI0.001hard_recessive'+str(hset[run])+'_model1_run1.out')
		lines=infile.readlines()
		infile.close()
		gen=[]
		polyscore=[]
